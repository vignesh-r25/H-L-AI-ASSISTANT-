    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = () => {
            if (document.hidden && state === 'LOCKED_IN') {
                // PENALTY LOGIC
                // In a real implementation we would decrease XP immediately
                toast.error("FOCUS BROKEN. PENALTY DETECTED.");
                // For now, let's just warn or deduct time
                // handlePenalty();
            }
        };
    ...
    if (state === 'IDLE') {
        ...
        return (
            <div className="min-h-screen bg-black text-blue-500 font-mono flex flex-col items-center justify-center p-4 relative overflow-hidden">
                ...
                <Button variant="ghost" className="absolute top-4 left-4 text-blue-900 hover:text-blue-500" onClick={() => navigate('/')}>
                    ← ABORT
                </Button>
            </div>
        );
        ...
        <div ref={containerRef} className="h-screen max-h-screen flex flex-col bg-slate-950 text-blue-400 font-mono overflow-hidden">
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-[70%] h-full border-r border-blue-900/20 bg-black relative">
                    <FocusReader onMaterialChange={(m) => setSelectedMaterial(m as any)} />
